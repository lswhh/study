...
unsigned int _hash(char* key, int len, int hashSize)
{
...
int hashInitialize(hashUtil * hashObj, int size)
{
    ...
    hashObj->hashTable = (utList*)malloc(sizeof(utList) * size);
...
int hashInsert(hashUtil * hashObj, char * key, void * data, int dataLen)
{
    ...
    hashData * sData;
    sLen = strlen(key);
    sHashValue = _hash(key, sLen, hashObj->size);
...
string solution(vector<string> aparticipant, vector<string> acompletion)
{
    char ** participant = aparticipant.data();
    int participantSize = aparticipant.size();
    char ** completion = acompletion.data();
    int completionSize = acompletion.size();
    utList sResultList;
    utListNode * sNode;
    utListNode * sDummy;
